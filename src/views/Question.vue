...
async function analyze () {
  ...
  loading = true
  const msgs = [{ role: 'user', content: input.textContent }]
...
<template>
  <div class="w-full min-h-screen flex flex-col items-center bg-gray-800 p-4 sm:p-10 text-white relative">
    ...
    <h1 class="font-bold text-3xl sm:text-5xl my-4">{{ lang ? '通用答疑' : 'General Question' }}</h1>
    <p class="mb-4">{{ lang ? '请在下面的框中输入您的问题' : 'Please input your question in the box below' }}</p>
    <pre ref="input" @input="checkReady" contenteditable class="font-mono bg-white text-black opacity-80 p-2 sm:p-4 w-full text-sm block my-6 min-h-[40vh] overflow-auto"></pre>
